...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        choice_lst = WOFComputerPlayer.getPossibleLetters(self, guessed)
        best_choice = list(WOFComputerPlayer.SORTED_FREQUENCIES[WOFComputerPlayer.SORTED_FREQUENCIES.index(c)] for c in choice_lst)

        if choice_lst == []:
            return 'pass'
        else:
            good_choice = WOFComputerPlayer.smartCoinFlip(self)
            if good_choice == True:
                good = best_choice[0]
                return good
            elif good_choice == False:
                bad = random.choice(choice_lst)
                return bad
